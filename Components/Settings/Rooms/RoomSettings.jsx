...
export default function RoomSettings() {
  ...
  const handlePropertyTypeChange = (value) => {
    ...
    const validTypes = ["room", "hall", "Room", "Hall"];
    ...
    setIsCreatingPropertyType(isValidType && !alreadyExists);
  };
  ...
  const handleSavePropertyType = async () => {
    ...
    const trimmedInput = propertyTypeInput.trim();
    const validTypes = ["room", "hall", "Room", "Hall"];

    // Check if input is one of the allowed types (case-insensitive)
    const isValidType = validTypes.some(
      (type) => type.toLowerCase() === trimmedInput.toLowerCase()
    );

    if (!isValidType) {
      toast.error("Property type must be either 'room' or 'hall'");
      setPropertyTypeInput("");
      return;
    }

    // Check if this type already exists (when creating new)
    if (!editingPropertyType) {
      const alreadyExists = propertyTypes.some(
        (type) => type.name.toLowerCase() === trimmedInput.toLowerCase()
      );

      if (alreadyExists) {
        toast.warning(`'${trimmedInput}' already exists`);
        setPropertyTypeInput("");
        return;
      }
    }
